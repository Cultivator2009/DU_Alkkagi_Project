# 세션 인계 노트 (최종 갱신 2026-09-24, Mac mini M4)
...
- **프로토콜 버전** (`NetMessage.ProtocolVersion`, 현재 1): 메시지 형식을 바꾸면 **반드시 올리세요.** 로비가 이 값을 광고하고, 다른 버전의 로비는 목록에 나오지 않으며 코드로 들어가도 거절됩니다("게임 버전이 달라 참가할 수 없어요"). 이 커밋에서도 `TurnResult`(킬 이벤트)와 `Kick` 메시지가 바뀌었습니다.
...
세 번째 테스트 피드백: 배치 중 "예외"는 Reporter 창을 착각한 것이었고, 키 바인딩은 정상입니다. 제한시간 직전 호스트 샷이 게스트에 반영되지 않던 문제는 바로 뒤 커밋에서 고쳤습니다(위 "턴 제한시간/스킵").
