...
- **장기판** (`Tools > Alkkagi > Build boards and janggi pieces`로 다시 생성): 판은 2.8×3.0이고, 9×10 줄과 궁성 2개를 그린 생성 텍스처를 씁니다. 접는 선(z=0) 위 x=±0.8에 **경첩 2개**가 있습니다.
  - 경첩은 판 위로 0.010 솟아 있습니다. 충돌체는 겉모습보다 넓은 캡슐을 판 속에 묻은 형태라 완만한 턱으로 작동합니다.
  - `HingeBump`가 닿아 있는 동안 위로 뜨는 속도를 0.4 m/s로 제한합니다.
  - 측정값: 최대 세기 샷은 11.2 → 5.4 m/s로 느려지며 0.009 튀어 넘어가고, 절반 세기 샷은 경첩에 걸려 멈춥니다. 조정 값은 빌더의 `proud`, `bumpRadius`, `HingeBump.maxHopSpeed`입니다.
- **장기말**: `JanggiPieceMesh`가 코드로 만드는 팔각형입니다. 위아래 모따기가 있고, 크기는 궁 / 차·포·마·상 / 사·졸·병 3가지입니다.
  - 글자는 한글이고, 주인 쪽을 향합니다. Pretendard에 한자가 없어서 한자로 바꾸려면 한자 폰트가 필요합니다.
...
- **개인 설정**(이 기기에만 저장): 언어, 전체 음량(`AudioListener`에 바로 적용), 인터페이스 음량(`GameSettings.InterfaceVolume`), 키 바인딩(`KeyBindings`)이 있습니다. 인터페이스 음량은 UI 효과음이 생기면 그 소리가 곱해서 쓰는 값이고, 아직 소리는 없습니다.
...
- 실제 마우스로 배치한 알 끌어 옮기기 (2026-09-24 테스트의 "예외"가 Reporter 창이었는지 확인. 계속 나면 Windows `Editor.log` 필요)
- 새 키 바인딩(Ctrl 시점 전환, 재지정)과 설정 창의 음량 슬라이더
